...
let visualPeak  = 0.01;
let visualFloor = 0;
const VISUAL_ADJUST_SPEED = 0.05;  // how fast visual range tracks actual AGC
...
const PEAK_HOLD_SEC = 5;              // ← tune this: how long to hold the peak before allowing decay
...
const FLOOR_WINDOW_SEC = 5;           // ← tune this: how long to remember the minimum
...
const panelLayoutDefs = [
  ...
  { key: 'media',       id: 'mediaPanel',   column: 2, order: 7  },
...
function updateCPU(c) {
  ...
  el.innerHTML = `
    ...
    ${barHTML(usage, 100, 'var(--cpu-bar-color)')}
...
function updateGPU(g) {
  ...
  if (usage != null) {
    ...
    html += barHTML(usage, 100, 'var(--gpu-bar-color)');
  }
  if (vramUsedMb != null && vramTotalMb != null) {
    ...
    html += barHTML(vramUsedMb, Math.max(vramTotalMb, 1), 'var(--vram-bar-color)');
  }
  html += `
...
function updateRAM(r) {
  ...
  el.innerHTML = `
    ...
    <div class="row"><span class="lbl">Usage</span><span class="val">${pct(usage)}</span></div>
    ${barHTML(usage, 100, 'var(--ram-bar-color)')}
...
function updateStorage(s) {
  ...
  let html = `
    ...
    ${barHTML(s.used_bytes || 0, s.total_bytes || 1, 'var(--storage-bar-color)')}
  `;
  for (const d of disks.slice(0, 3)) {
    ...
    html += barHTML(pUsed, 100, 'var(--storage-pool-bar-color)');
  }
  el.innerHTML = html;
}
...
function updateAudioStats(a) {
  ...
  el.innerHTML = `
    ...
    ${barHTML((lvl.smoothed_peak || 0) * 100, 100, 'var(--audio-bar-color)')}
  `;
}
...
function updatePower(p) {
  ...
  if (pct_val != null) {
    ...
    html += barHTML(pct_val, 100, 'var(--battery-bar-color)');
  } else if (hasBattery) {
...
function updateWifi(w) {
  ...
  if (connected) {
    ...
    if (signal != null) {
      ...
      html += barHTML(signal, 100, 'var(--wifi-bar-color)');
    }
...
function renderMediaSession(ms) {
  ...
  if (!ms || (ms.playing === false && !ms.title)) {
    el.innerHTML = `<div class="media-idle">
  ...
  /* ── Album art ── */
  const artHTML = ms.thumbnail
    ? `<img class="media-art" src="${ms.thumbnail}" alt="">`
    : `<div class="media-art-placeholder"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v10.55A4 4 0 1 0 14 17V7h4V3h-6Z"/></svg></div>`;

  /* ── Track info ── */
  let infoHTML = '';
  if (ms.title)  infoHTML += `<div class="media-title">${ms.title}</div>`;
  const artistText = ms.artist || ms.album_artist || '';
  if (artistText) infoHTML += `<div class="media-artist">${artistText}</div>`;
  if (ms.album && ms.album !== artistText) infoHTML += `<div class="media-album">${ms.album}</div>`;
  infoHTML += `<div class="media-status">
    <span class="media-status-dot${isPlaying ? ' playing' : ''}"></span>
    <span class="media-status-label">${status}</span>
  </div>`;
  ...
  if (ms.timeline && ms.timeline.duration_ms > 0) {
    ...
    progressHTML = `<div class="media-progress">
      <div class="media-progress-bar"><div class="media-progress-fill" style="width:${pct.toFixed(1)}%"></div></div>
      <div class="media-time"><span>${fmtTimeMs(pos)}</span><span>${fmtTimeMs(dur)}</span></div>
    </div>`;
  }
  ...
  const chips = [];
  if (ms.source_app_id) {
    const app = ms.source_app_id.replace(/\.exe$/i, '');
    chips.push(`<span class="media-chip">${app}</span>`);
  }
  if (ms.playback_type && ms.playback_type !== 'unknown')
    chips.push(`<span class="media-chip">${ms.playback_type}</span>`);
  if (ms.shuffle === true)
    chips.push(`<span class="media-chip active">shuffle</span>`);
  if (ms.repeat_mode && ms.repeat_mode !== 'none')
    chips.push(`<span class="media-chip active">repeat ${ms.repeat_mode}</span>`);
  if (ms.genres && ms.genres.length)
    chips.push(`<span class="media-chip">${ms.genres.join(', ')}</span>`);
  if (ms.track_number)
    chips.push(`<span class="media-chip">Track ${ms.track_number}${ms.album_track_count ? '/' + ms.album_track_count : ''}</span>`);
  const metaHTML = chips.length ? `<div class="media-meta">${chips.join('')}</div>` : '';

  /* ── Assemble ── */
  el.innerHTML = `<div class="media-hero">${artHTML}<div class="media-info">${infoHTML}</div></div>${progressHTML}${metaHTML}`;
}
...
Sentinel.subscribe('mouse', m => {
  ...
  if (cursor && Number.isFinite(cursor.x) && Number.isFinite(cursor.y)) {
    // Map absolute desktop-pixel cursor to this monitor's 0–1 local space.
    // monitorBounds is injected by the wallpaper addon on every tick BEFORE
    // registry data, so it is always available by the time this fires.
    // If somehow missing, skip cursor update entirely — the virtual-screen
    // fallback would produce wrong coordinates on non-primary monitors.
    const mb = Sentinel.monitorBounds;
    if (!mb) return;   // bounds not yet received; wait for next tick

    const nx = Math.max(0, Math.min(1, (Number(cursor.x) - mb.left) / Math.max(1, mb.width)));
    const ny = Math.max(0, Math.min(1, (Number(cursor.y) - mb.top) / Math.max(1, mb.height)));
